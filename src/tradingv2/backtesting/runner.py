...
from tradingv2.config import BacktestConfig
from tradingv2.data.store import (
    MarketDataEntry,
    MarketDataStore,
    combine_frames,
    entry_end_date,
...
from tradingv2.data.validation import validate_ohlcv
from tradingv2.exceptions import DataError, NoDataError, StrategyError, TradingV2Error
...
class BacktestRunner:
    ...
    def _execution_price(
        ...
        raise TradingV2Error(f"unsupported execution price: {price_mode}")
    ...
    def _sizing_kwargs(self, close: pd.DataFrame, entries: pd.DataFrame, signals: dict) -> dict:
        ...
        raise TradingV2Error(f"unsupported sizing method: {sizing.method}")
